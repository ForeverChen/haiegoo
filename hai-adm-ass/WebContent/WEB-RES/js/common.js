...
if(Ext){
	/**
	 * 允许grid复制 override the [Ext.grid.Panel]
	 */
	...
	/**
	 * 将Viewport底色改为白色，默认是蓝色 override the [Ext.grid.Panel]
	 */
	...
	/**
	 * grid、tree等sotre加载时异常处理
	 */
	Ext.storeException = function(proxy, request, operation, options) {
	...
	Ext.formFailure = function(form, action) {
		...
		case Ext.form.action.Action.CLIENT_INVALID:
			Ext.Msg.alert('提示', '输入的内容有误');
			break;
		case Ext.form.action.Action.CONNECT_FAILURE:
			if (action.response.status == 403) {
				Ext.Msg.alert('提示', '您没有权限访问');
			} else {
				Ext.Msg.alert("提示", action.response.responseText);
			}
			break;
