...
Ext.define('Ext.ux.TreePicker', {
    ...
    setValue: function(value) {
        ...
        //Linpn: 修改让treepicker选择一个节点后,显示树的层级，如：一级分类 > 二级分类 > 三级分类
        if(record){
			var rawValue = record.data.name;			
			var pid = record.data.pid;
			while(pid && pid>0){
				var _record = me.store.getNodeById(pid);
				pid = _record.data.pid;
				rawValue = _record.data.name + " > " + rawValue;
			}
        }else{
        	rawValue = '';
        }
